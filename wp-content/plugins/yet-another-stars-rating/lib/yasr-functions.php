...
/*** Customize message for new install ***/

yasr_fs()->add_filter('connect_message', 'yasr_fs_custom_connect_message', 10, 6); 

function yasr_fs_custom_connect_message(
        $message,
        $user_first_name,
        $plugin_title,
        $user_login,
        ...
        return sprintf(
            __fs( 'hey-x' ) . '<br>' .
            __( 'Hey %1$s, in order to enjoy all our features and functionality, %2$s needs to connect your user, %3$s at %4$s, to freemius.com.' ),
            $user_first_name,
            '<b>' . $plugin_title . '</b>',
            '<b>' . $user_login . '</b>',
            $site_link,
...
yasr_fs()->add_filter('connect_message_on_update', 'yasr_fs_custom_connect_message_on_update', 10, 6);


function yasr_fs_custom_connect_message_on_update(
        $message,
        $user_first_name,
        $plugin_title,
        $user_login,
        ...
        return sprintf(
            __fs( 'hey-x' ) . '<br>' .
            __( 'Please help us improve %2$s! If you opt-in, some data about your usage of %2$s will be sent to %5$s. If you skip this, that\'s okay! %2$s will still work just fine.', 'yet-another-stars-rating' ),
            $user_first_name,
            '<b>' . $plugin_title . '</b>',
            '<b>' . $user_login . '</b>',
            $site_link,
