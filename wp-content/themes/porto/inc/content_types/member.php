...
// remove meta from custom fields meta box
...
function porto_member_protected_meta($protected, $meta_key, $meta_type) {
    if (!$protected) {
        global $member_meta_boxes, $member_view_meta_boxes, $member_skin_meta_boxes;
        if (array_key_exists($meta_key, $member_meta_boxes))
            $protected = true;
        if (array_key_exists($meta_key, $member_view_meta_boxes))
            $protected = true;
        if (array_key_exists($meta_key, $member_skin_meta_boxes))
            $protected = true;
    }
    return $protected;
}
